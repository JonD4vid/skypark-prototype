<template>
  <div class="screen">
    <button class="back-btn" type="button" @click="$router.push('/account/topup')">‹ Back</button>
    <h1>Pay {{ store.topUpDue }}</h1>
    <p class="lede">Prepaid credit · current balance {{ money(store.creditBalance) }}</p>
    ...
    <div v-if="!store.cards.length || useNew" class="card">
      ...
      <div style="padding:16px 16px 18px;display:flex;flex-direction:column;gap:12px">
        ...
        <label style="display:flex;align-items:center;gap:8px;font:500 13.5px var(--font)">
    ...
    <button class="btn btn-primary" type="button" @click="payWithCard">Pay {{ store.topUpDue }}</button>
...
const useNew = ref(!store.cards.length)

onMounted(() => {
  if (!store.topUpAmount) {
    router.replace('/account/topup')
    return
  }
  store.paying = false
  store.paymentPurpose = 'topup'
  if (store.cards.length && !store.selectedCardId) {
    store.selectedCardId = store.cards[0].id
  }
  if (store.cards.length && !useNew.value) {
...
function pay(method) {
  const result = store.startPayment(method, 'topup')
